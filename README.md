...
The examples below assume the following input files:

- `sample.asm.bp.p_ctg.gfa`: assembly graph generated by **hifiasm**
- `sample_plastid.fa`: plastid / chloroplast reference FASTA
- `sample_mito.fa`: mitochondrial reference FASTA

### Minimal graph-only run

```bash
organelle-cleaner sample.asm.bp.p_ctg.gfa \
  --mode graph-only \
  --output-dir graph_only_results \
  --threads 8
```
...
```bash
organelle-cleaner sample.asm.bp.p_ctg.gfa \
  --plastid-fasta sample_plastid.fa \
  --mode hybrid \
...
If your GFA does not include usable contig sequences, you can provide `--assembly-fasta` explicitly. If you need a separate candidate-only report file, `--all-candidates-name` remains available as an advanced option.
